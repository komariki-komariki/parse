...
def text_article():
    HEADERS = {
        'Cookie': '_ym_uid=1639148487334283574; _ym_d=1639149414; _ga=GA1.2.528119004.1639149415; _gid=GA1.2.512914915.'
                  '1639149415; habr_web_home=ARTICLES_LIST_ALL; hl=ru; fl=ru; _ym_isad=2; __gads=ID=87f529752d2e0de1-'
    ...
    for data in articles:
        ...
        try:
            texts = data.find('div', class_="article-formatted-body article-formatted-body_version-2").text.strip().\
                replace('\n','').replace('\xa0','').replace('\r','')
        except:
            texts = data.find('div', class_="article-formatted-body article-formatted-body_version-1").text.strip().\
                replace('\n','').replace('\xa0','').replace('\r','')
...
def search():
    if KEYWORDS[0] in articles_list[0][2]:
        print(articles_list[0][-1])
    else:
        print('не найдено')
